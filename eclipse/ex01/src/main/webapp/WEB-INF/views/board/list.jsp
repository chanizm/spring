			...
			<div class="panel-body">
				<table class="table table-striped table-bordered table-hover">
					...
					<c:forEach items="${list}" var="board">
						<tr>
							<td><c:out value="${board.message_id}" /></td>
							<%-- <td><a href='/board/get?message_id=<c:out value="${board.message_id}"/>'><c:out value="${board.title}"/></a></td> --%>

							<td><a class='move' href='<c:out value="${board.message_id}"/>'>
									<c:out value="${board.guest_name}" />
							</a></td>

							<td><c:out value="${board.message}" /></td>
					...
					<div class="col-lg-12">
						...
						<form id='searchForm' action="/board/list" method='get'>
							<select name='type'>
								...
								<option value="C"
									<c:out value="${pageMaker.cri.type eq 'C'?'selected':''}"/>>비번</option>
								<option value="W"
									<c:out value="${pageMaker.cri.type eq 'W'?'selected':''}"/>>메시지</option>
					...
					function() {
						...
						$(".paginate_button a").on(
								...
								function(e) {
									...
									actionForm.find("input[name='currentPageNo']")
											.val($(this).attr("href"));
									actionForm.submit();
								});
						...
						$(".move")
								.on(
										...
										function(e) {
											...
											e.preventDefault();
											actionForm
													.append("<input type='hidden' name='message_id' value='"
															+ $(this).attr(
																	"href")
															+ "'>");
